  ...
  const peopleFiltered = people
    .filter((person) => {
      if (searchTerm === "") {
        ...
        `${person.user.firstName.toLowerCase()} ${person.user.lastName.toLowerCase()}`.includes(
    ...
    .map((person) => {
      return <PeopleItem people={people} person={person} key={person._id} />;
    });

  return (
    <div className="people__people">
      <h3>People</h3>
      <div className="people__people--list">
        <div className="people__people--peopleItem">
          <svg
